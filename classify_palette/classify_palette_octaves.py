...
For each .wav file in --in_dir, we:
  ...
  - create three versions:
        'orig'  : original
        'down2' : shifted -24 semitones
        'up2'   : shifted +24 semitones
...
def main():
    ...
    # but we know from training that it's constant; easiest is to infer
    ...
    model = AudioCNNv6(n_classes=num_classes,
        ...
        for wav_path in wavs:
            ...
            for variant_name, semitones in variants.items():
                if semitones == 0:
                    ...
                    try:
                        y_var = librosa.effects.pitch_shift(
                            y, sr=TARGET_SR, n_steps=semitones
                        )
                    except Exception as e:
                        print(f"  [WARN] pitch_shift {variant_name} failed: {e}")
                        continue
                ...
                print(f"  {variant_name}: {primary_label} ({primary_conf:.3f}) "
