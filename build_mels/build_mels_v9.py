...
def classify_mdlib(path: Path) -> Optional[str]:
    name = path.name.lower()
    parent = str(path.parent).lower()
    text = f"{name} {parent}"

    if any(k in text for k in ["snare"]):
        return "snare"
    if any(k in text for k in ["tom", "racktom", "floortom", "rt", "ft"]):
        return "toms"
    if any(k in text for k in ["kick", "bassdrum", "bd"]):
        return "kick"
    if any(k in text for k in ["hihat", "hi-hat", "hh", "closedhat", "openhat"]):
        return "hihat"
    if any(k in text for k in ["cym", "cymbal", "ride", "crash", "splash", "china"]):
        return "cymbals"
    return None


def iter_mdlib_audio() -> Iterable[Tuple[str, str, Path]]:
    root = BASE / "mdlib" / "MDLib2.2"   # adjust if `ls` shows a slightly different name
    wavs = list(root.rglob("*.wav"))
    print(f"[MDLib] Found {len(wavs)} .wav files under {root}")
    for w in wavs:
        lab = classify_mdlib(w)
        if lab is None or lab not in CLASS_TO_IDX:
            continue
        yield ("mdlib_perc", lab, w)
